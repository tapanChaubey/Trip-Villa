...
const Navbar = () => {
  ...
  return (
    <nav className={`sticky top-0 left-0 w-full z-50 py-4 px-4 md:px-12 lg:px-20 xl:px-32 backdrop-blur-md transition-all duration-500 flex items-center justify-between ${
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        {user ? (
          <>
            <Link to="/cart" className="relative flex items-center p-2 rounded-full hover:bg-indigo-100 transition">
              <svg className="h-6 w-6 text-blue-700 hover:text-indigo-600 transition" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M3 3h2l.4 2M7 13h10l4-8H5.4" />
                <circle cx="7" cy="21" r="1" />
                <circle cx="17" cy="21" r="1" />
              </svg>
              {cartCount > 0 && (
                <span className="absolute top-0 right-0 transform translate-x-1 -translate-y-1 bg-red-500 text-white text-xs font-semibold px-1.5 py-0.5 rounded-full">
                  {cartCount}
                </span>
              )}
            </Link>
      ...
      <div className="flex items-center md:hidden">
        <button onClick={() => setIsMenuOpen(true)} className="p-2 rounded hover:bg-gray-200 transition">
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm flex justify-end">
          <div className="w-4/5 sm:w-2/3 md:w-1/2 bg-white h-full p-6 flex flex-col overflow-y-auto scroll-smooth overscroll-contain shadow-lg transition-transform duration-300 ease-in-out">
            {/* Top Section */}
            <div className="flex flex-col gap-6">
              ...
              <Link to="/" onClick={() => setIsMenuOpen(false)} className="text-center text-2xl font-bold tracking-wide text-gray-800">
                <span className="text-rose-600">Trip</span>
                <span className="text-indigo-700">+Villa</span>
              </Link>
